...
void Neighbor::half_bin_newton(NeighList *list)
{
  ...
  //printf (" Doing Neighbor Computation!  cnsq=%f \n", cutneighsq[1][2] );
  ...
  for (i = 0; i < nlocal; i++) {
	...
	if (x[j][2] == ztmp && x[j][1] == ytmp && x[j][0] < xtmp) continue;
      ...
      if (rsq <= cutneighsq[itype][jtype]) {
        //printf("adding pair1 %d %d  find_special()=%d\n", itype,jtype, find_special(i,j));
	...
	if (rsq <= cutneighsq[itype][jtype]) {
    //printf("adding pair2 %d %d  find_special()=%d\n", itype,jtype, find_special(i,j));
  ...
  //printf (" End Neighbor Computation!  n=%d inum=%d \n", n, inum );
  list->inum = inum;
}
