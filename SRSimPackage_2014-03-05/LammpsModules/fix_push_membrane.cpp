 ...
 */
FixPushMembrane::FixPushMembrane(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
void FixPushMembrane::init()
{
  if (strcmp(update->integrate_style,"respa") == 0)
     error->all("FixPushMembrane::setup: Sorry, I don't know about respa...");
}

/* ---------------------------------------------------------------------- */

void FixPushMembrane::setup(int vflag)
{
  if (strcmp(update->integrate_style,"verlet") == 0)
    post_force(vflag);
  else {
    error->all("FixPushMembrane::setup: Sorry, I can only do verlet simulations...");
  }
}
 ...
 */
void FixPushMembrane::post_force(int vflag)
{
  ...
  for (int i = 0; i < nlocal; i++)
    if (mask[i] & groupbit) {
      ...
      double posInCurve     = (x[i][0] - (xStart + membraneLength)) / membraneLength * 2.0;  // first map xStart and xEnd to +/- 5
      ...
      if (r - (yzRad+addCurve) > 0) continue;  // we're outside anyway... ignore.
