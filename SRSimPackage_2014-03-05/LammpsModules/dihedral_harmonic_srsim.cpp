...
DihedralHarmonicSRSim::DihedralHarmonicSRSim(LAMMPS *lmp) : Dihedral(lmp) {}

/* ---------------------------------------------------------------------- */

DihedralHarmonicSRSim::~DihedralHarmonicSRSim()
{
  if (allocated) {
...
void DihedralHarmonicSRSim::compute(int eflag, int vflag)
{
    
  //if (update->ntimestep > 351100) printf("DihedralHarmonicSRSim::compute - Hallo - ts = %d.\n", update->ntimestep );
  ...
  for (n = 0; n < ndihedrallist; n++) {
    ...
    // c,s calculation           // Gerdl: that would be the two cross products?
  ...
  //if (update->ntimestep > 351100) printf("DihedralHarmonicSRSim::compute - Done. - ts = %d.\n", update->ntimestep );
  
}

/* ---------------------------------------------------------------------- */

void DihedralHarmonicSRSim::allocate()
{
  allocated = 1;
...
/**
 *    Inserted one further parameter, to set the sin_shift / cos_shift values!
 */
void DihedralHarmonicSRSim::coeff(int which, int narg, char **arg)
{
  if (which > 0) return;
  ...
  double alpha_one = force->numeric(arg[4]);
  ...
  for (int i = ilo; i <= ihi; i++) {
    ...
    // sin/cos shift: borrowed from Dihedral-Charmm:
    cos_shift[i] = cos(PI + PI*alpha_one/180.0);
    sin_shift[i] = sin(PI + PI*alpha_one/180.0);
  }
...
void DihedralHarmonicSRSim::write_restart(FILE *fp)
{
...
void DihedralHarmonicSRSim::read_restart(FILE *fp)
{
  allocate();
